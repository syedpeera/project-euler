...
int i,j;
unsigned long long big=0,temp1=0,temp2=0,temp3=0,temp4=0;
for(i=0;i<20;i++)
{
	for(j=0;j<20;j++)
	{
		...
		temp3=a[i][j]*a[i+1][j]*a[i+2][j]*a[i+3][j];
		if(big<temp3)
		{
			big=temp3;
		}
		temp4=a[i][j]*a[i][j+1]*a[i][j+2]*a[i][j+3];
		if(big<temp4)
		{
			big=temp4;
		}
...
cout<<"The greatest product of numbers in the array is : "<<big;	
return 0;	
}
